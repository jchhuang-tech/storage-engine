...
class SkipListTest : public ::testing::Test {
 protected:
  ...
  void NewSkipList(uint32_t key_size) {
    slist = new SkipList(key_size);
  }
};
...
TEST_F(SkipListTest, NewNodeTooHigh) {
  ...
  SkipListNode *node = slist->NewNode(100, key.c_str(), rid);
  ASSERT_FALSE(node);
  free(node);
}
...
TEST_F(SkipListTest, NewNode) {
  ...
  SkipListNode *node = slist->NewNode(4, key.c_str(), rid);
  ASSERT_TRUE(node);
